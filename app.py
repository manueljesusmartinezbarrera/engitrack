...
with tab_pomo:
    ...
    with col_m1:
        if st.button("Pomodoro 20", key="pomo_20_btn", use_container_width=True):
            ...
            st.rerun()
    with col_m2:
        if st.button("Descanso 5", key="desc_5_btn", use_container_width=True):
            st.session_state.timer_running = False
            ...
            st.rerun()
    with col_m3:
        if st.button("Descanso largo 15", key="desc_15_btn", use_container_width=True):
            st.session_state.timer_running = False
    ...
    if st.session_state.modo_actual == "Pomodoro":
        ...
        if st.button(f"💾 Guardar {mins_reales} min en la nube", key="save_pomo_db_btn", use_container_width=True):
            if db_ok:
                try:
                    with conn.session as s:
